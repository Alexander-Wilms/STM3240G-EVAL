...
#define portUSING_FPU_MPU			1
#define portFPU_BIT					( 0x40000000UL )
...
#define portEND_SWITCHING_ISR( xSwitchRequired ) if( xSwitchRequired ) { *(portNVIC_INT_CTRL) = portNVIC_PENDSVSET; LEAVE_ISR} else LEAVE_ISR;
...
__attribute__( ( always_inline ) ) static inline unsigned ucPortCountLeadingZeros( unsigned long ulBitmap )
{
	unsigned ucReturn;
