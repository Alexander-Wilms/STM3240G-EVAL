...
#if ( portUSING_FPU_MPU == 1 )            /*< This flag tells the context switch mechanism to take care of the FPU context also */
	UBaseType_t taskUsingFPU;
#endif
...
StackType_t *pxTopOfStack;
	...
	configASSERT( ( ( uxPriority & ( UBaseType_t ) ~( portPRIVILEGE_BIT | portFPU_BIT) ) < ( UBaseType_t ) configMAX_PRIORITIES ) );
	...
	if( pxNewTCB != NULL )
	{
		...
		#else /* portSTACK_GROWTH */
		{
		  ...
		  pxNewTCB->taskUsingFPU = (uxPriority & portFPU_BIT) != 0 ? 1 : 0;
		  uxPriority &= ~portFPU_BIT;
		...
		#if( portUSING_FPU_MPU == 1)
		{
			pxNewTCB->pxTopOfStack = pxPortInitialiseStack( pxTopOfStack, pxTaskCode, pvParameters, xRunPrivileged, pxNewTCB->taskUsingFPU);
	      	}
		#elif( portUSING_MPU_WRAPPERS == 1 )
		{
